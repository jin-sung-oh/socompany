import type { Agent, AgentStatus, AnimalSpecies, AppSettings, Persona } from "@kafi/shared";
...
const toneFallbackBySpecies: Record<AnimalSpecies, Persona["tone"]> = {
...
export const SPECIES_METADATA: Record<AnimalSpecies, SpeciesMetadata> = {
  capybara: {
    label: "카피바라",
    emoji: "🦫",
...
export const cloneAgent = (agent: Agent): Agent => ({
  ...agent,
...
export const normalizeAgents = (storedAgents: unknown): Agent[] => {
  ...
  const candidates = Array.isArray(storedAgents) ? storedAgents : [];
  const usedIndexes = new Set<number>();

  return defaults.map((defaultAgent) => {
    let matchedIndex = candidates.findIndex((candidate, index) => {
      if (usedIndexes.has(index) || !candidate || typeof candidate !== "object") {
        return false;
      }
      return (candidate as Partial<Agent>).id === defaultAgent.id;
    });
    ...
    if (matchedIndex === -1) {
      matchedIndex = candidates.findIndex((candidate, index) => {
        ...
        const next = candidate as Partial<Agent>;
        return next.species === defaultAgent.species;
      });
    }
      ...
      persona: {
        description:
          typeof matched.persona?.description === "string" && matched.persona.description.trim()
            ? matched.persona.description
            : defaultAgent.persona?.description ?? "",
        tone: matched.persona?.tone ?? toneFallbackBySpecies[defaultAgent.species],
        instructions:
          matched.persona?.instructions?.filter((instruction) => typeof instruction === "string" && instruction.trim()) ??
...
export const getSpeciesMeta = (species: AnimalSpecies) => SPECIES_METADATA[species];

export const getAgentAsset = (agent: Agent, status?: AgentStatus): string => {
  switch (status ?? agent.status) {
    case "working":
      return agent.character.workingAsset;
    case "thinking":
      return agent.character.thinkingAsset;
    case "completed":
      return agent.character.completedAsset;
    case "error":
      return agent.character.errorAsset;
    default:
      return agent.character.idleAsset;
  }
};

export const buildAgentPrompt = (agent: Agent): string => {
  const meta = getSpeciesMeta(agent.species);
